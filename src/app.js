...
(function () {
  ...
  // TEMPORARY DIAGNOSTIC FLAG — set to false to fully bypass the awakening
  // fade-in and rule it out as a cause of the plane instability. Once base
  // tracking is confirmed stable with this off, we re-enable and re-test
  // in isolation.
  const ENABLE_AWAKEN_FADE = false;
  ...
  const ARController = {
    ...
    buildTargetEntities(paintings) {
      paintings.forEach((painting) => {
        ...
        // Only pre-set transparency/opacity if the fade effect is active.
        // When disabled, the plane behaves exactly like the original
        // prototype: fully opaque, no material overrides at all.
        if (ENABLE_AWAKEN_FADE) {
    ...
    playVideo(ref) {
      ...
      if (!ENABLE_AWAKEN_FADE) {
        // Exact original behavior: instantly visible, no material touched.
        return;
      }
